...
namespace VSP.Presentation.Forms
{
    public partial class frmAdvisor : Form, IMessageFilter
    {
        ...
        public Advisors CurrentAdvisor;

        public frmAdvisor(frmMain mainForm)
        {
            InitializeComponent();
            mf_parent = mainForm;
        ...
        public frmAdvisor(frmMain frmMain_Parent, Guid advisorId)
        {
            ...
            CurrentAdvisor = new Advisors(advisorId);
            ...
            try
            {
                ...
                foreach (DataRow dr in VSP.Business.Entities.Advisors.GetAdvisorDetails(advisorId).Rows)
                {
            ...
            dgvFunds.DataSource = VSP.Business.Entities.Fund.GetAssociatedFromAdvisor(advisorId);
            dgvFunds.Columns["Relational_Advisors_FundsId"].Visible = false;
            dgvFunds.Columns["FundId"].Visible = false;

            dgvMgrs.DataSource = VSP.Business.Entities.Manager.GetAssociatedFromAdvisor(advisorId);
        ...
        private bool Save()
        {
            ...
            if (cboResearchType.SelectedItem != null && !String.IsNullOrEmpty(((Utilities.ListItem)cboResearchType.SelectedItem).HiddenValue))
            {
            ...
            CurrentAdvisor.Name = Name;
            CurrentAdvisor.CompanyFounded = CompanyFounded;
            CurrentAdvisor.InvestmentPhilosophy = InvestmentPhilosophy;
            CurrentAdvisor.InterestConflicts = InterestConflicts;
            CurrentAdvisor.ShareholderRights = ShareholderRights;
            CurrentAdvisor.CompensationStructure = CompensationStructure;
            CurrentAdvisor.IsCultureProfessional = IsCultureProfessional;
            CurrentAdvisor.IsCultureTransparent = IsCultureTransparent;
            CurrentAdvisor.IsCultureMoralStandardsHigh = IsCultureMoralStandardsHigh;
            CurrentAdvisor.IsResearchProprietary = IsResearchProprietary;
            CurrentAdvisor.IsResearchQuantitative = IsResearchQuantitative;
            CurrentAdvisor.IsResearchFundamental = IsResearchFundamental;
            CurrentAdvisor.ResearchType = ResearchType;
            CurrentAdvisor.StaffSize = StaffSize;
            CurrentAdvisor.StaffCredentials = StaffCredentials;
            CurrentAdvisor.StaffExperience = StaffExperience;
            CurrentAdvisor.AnalystRatio = AnalystRatio;
            CurrentAdvisor.AssetsFixedIncome = AssetsFixedIncome;
            CurrentAdvisor.AssetsEquity = AssetsEquity;
            CurrentAdvisor.AssetsOther = AssetsOther;

            CurrentAdvisor.SaveRecordToDatabase(UserId);
        ...
        public void UpdateResearchTypeCbo()
        {
            ...
            {
                cboResearchType.Items.Add(new ListItem(_stringMap.Value, _stringMap.Id.ToString()));
            }
        }
        ...
        private void btnMgmtUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (String.IsNullOrWhiteSpace(txtMgmt.Text))
            {
                MessageBox.Show("Management member name cannot be blank. Please correct and try again.", "Error!", MessageBoxButtons.OK);
                return;
            }
        ...
        private void LoadManagementRolesCbo()
        {
            ...
            {
                cboMgmtRoles.Items.Add(new ListItem(_stringMap.Value, _stringMap.Id.ToString()));
            }
        }
        ...
        private void btnMgmtRoleUpdate_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("You must select a member role to update. Please correct and try again.", "Error!", MessageBoxButtons.OK);
                return;
            }

            if (cboMgmtRoles.SelectedIndex == -1)
            {
                MessageBox.Show("You must select a member role from the drop down box to update. Please correct and try again.", "Error!", MessageBoxButtons.OK);
                return;
            }
        ...
        {
            if (lstMgmt.SelectedItem == null)
            {
                MessageBox.Show("You must select a management member to add a new role. Please correct and try again.", "Error!", MessageBoxButtons.OK);
                return;
            }

            if (cboMgmtRoles.SelectedIndex == -1)
            {
                MessageBox.Show("You must select a member role from the drop down box to save a new record. Please correct and try again.", "Error!", MessageBoxButtons.OK);
                return;
            }
        ...
        private void btnMgmtRoleDelete_Click(object sender, EventArgs e)
        {
            if (lstMgmtRoles.SelectedIndex == -1 || lstMgmtRoles.SelectedItem == null)
            {
                MessageBox.Show("You must select a member role to delete. Please correct and try again.", "Error!", MessageBoxButtons.OK);
                return;
            }
