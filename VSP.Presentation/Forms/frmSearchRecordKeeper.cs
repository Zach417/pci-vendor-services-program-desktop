...
{
	public partial class frmSearchRecordKeeper : Form, IMessageFilter
    {
        ...
        public frmSearchRecordKeeper(frmMain mf, Search search, FormClosedEventHandler Close = null)
        {
        ...
        /// <param name="Close"></param>
        public frmSearchRecordKeeper(frmMain mf, SearchRecordKeeper searchRecordKeeper, FormClosedEventHandler Close = null)
        {
            frmSplashScreen ss = new frmSplashScreen();
            ss.Show();
            Application.DoEvents();
            ...
            CurrentSearchRecordKeeper = searchRecordKeeper;
        ...
        {
            if (String.IsNullOrWhiteSpace(txtOrdinal.Text))
            {
                MessageBox.Show("Error: Ordinal string cannot be left blank");
                return;
            }
            else
            {
                try
                {
                    CurrentSearchRecordKeeper.Ordinal = int.Parse(txtOrdinal.Text);
                }
                catch
                {
                    MessageBox.Show("Error: Ordinal string not in integer format");
                    return;
                }
            }
            ...
            {
                MessageBox.Show("Error: Selected record keeper in record keeper list box cannot be left blank");
                return;
            }
            else
            {
                ...
                CurrentSearchRecordKeeper.RecordKeeperId = recordKeeperId;
            }

            CurrentSearchRecordKeeper.SaveRecordToDatabase(frmMain_Parent.CurrentUser.UserId);
            //this.Close();
        }

        private void txtName_TextChanged(object sender, EventArgs e)
        {
            label23.Text = txtOrdinal.Text;
        }
	}
}
