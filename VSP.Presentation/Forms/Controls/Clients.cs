...
namespace VSP.Forms.Controls
{
    public partial class Clients : UserControl
    {
        ...
        private void LoadAutoCompleteClients()
        {
            ...
            foreach (DataRow dr in UserSearches.GetAssociatedFromTable(frmMainParent.CurrentUser.UserId, "Accounts").Rows)
            {
        ...
        private void txtSearchClnts_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (!String.IsNullOrWhiteSpace(txtSearch.Text))
                {
                    ...
                    userSearch.SearchText = txtSearch.Text;
                    userSearch.SearchTable = "Accounts";
                    userSearch.SaveRecordToDatabase(frmMainParent.CurrentUser.UserId);
